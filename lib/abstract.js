...
Abstract.prototype.create = function(callback) {
  this.request('POST', [this.path, this.id], this.getData(), function(err, resp, body) {
    if (callback) {
...
Abstract.prototype.request = function(method, uri) {
  ...
  options = merge(this.options,  { 'method': method, 'url': url, 'body': (method.toUpperCase() === 'POST' ? json : '') } )
...
Abstract.prototype.requestList = function(instanceObject, method, uri) {
  var _this = this
  return function() {
    // first argument is the options to the instanceObject
