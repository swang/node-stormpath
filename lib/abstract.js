...
Abstract.prototype.request = function(method, uri) {
  ...
  options = merge(this.options,  { 'method': method, 'url': url, 'followRedirect': false })
  this.req(options, function(err, resp, body) {
    if (err) { return callback(err) }
    if (resp.statusCode >= 400) {
      if (_this.statusCodes[resp.statusCode]) {
        return callback(new Error([resp.statusCode, _this.statusCodes[resp.statusCode].message]))
      }
      else {
        return callback(new Error([resp.statusCode, body.message]))
      }
    }
  ...
  return function() {
    ...
    _this.request(method, uri, json, function(err, resp, body) {
        ...
        expands.forEach(function(resource) {
          if (body[resource] && body[resource].items) {
            ...
            body[resource].items.forEach(function(resItem) {
              resSingle.push(new global[h.titleCase(resource)](_this.options).setData(resItem))
            })
      ...
      if (body.items) {
        ...
        return callback(null, new instanceObject(_this.options).setData(body))
      }
